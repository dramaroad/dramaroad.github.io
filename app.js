...
function ageLabel(t){return t.ageText||`${t.age} سنة`}
function trainingLabel(t){return t.trainingYears===0.5?'6 أشهر':`${t.trainingYears} سنوات`}
...
function talentCard(t){const skills=t.skills?.length?t.skills.map(s=>`<span class="chip">${esc(s)}</span>`).join(''):'';return `<article class="talent-card" data-id="${esc(t.id)}"><div class="talent-photo"><img src="${esc(t.image||'logo.webp')}" alt="${esc(t.name)}" onerror="this.src='logo.webp'"><span class="years-badge">${trainingLabel(t)} في المعهد</span></div><div class="talent-body"><h3>${esc(t.name)}</h3><div class="meta"><span class="chip red">${ageLabel(t)}</span>${t.heightCm?`<span class="chip">${t.heightCm} سم</span>`:''}${skills}</div><p class="talent-note">${esc(t.notes||'Drama Road Young Talent')}</p></div></article>`}
...
function openProfile(id){const t=talents.find(x=>x.id===id);if(!t)return;const tp=projects.filter(p=>p.talentId===id);$('#profileContent').innerHTML=`<div class="profile-wrap"><div class="profile-image"><img src="${esc(t.image||'logo.webp')}" alt="${esc(t.name)}" onerror="this.src='logo.webp'"></div><div class="profile-info"><span class="eyebrow">DRAMA ROAD TALENT</span><div class="profile-title-row"><h2>${esc(t.name)}</h2>${tp.length?`<button class="profile-projects-jump" type="button" onclick="document.getElementById('profileProjectsSection').scrollIntoView({behavior:'smooth',block:'start'})">▶ مشاهدة المشاريع</button>`:''}</div><div class="meta"><span class="chip red">${ageLabel(t)}</span><span class="chip">${trainingLabel(t)} دراسة</span>${t.heightCm?`<span class="chip">${t.heightCm} سم</span>`:''}${t.gender?`<span class="chip">${esc(t.gender==='أنثى'?'بنت':'ولد')}</span>`:''}</div><div class="profile-details"><div class="detail"><small>العمر</small><b>${ageLabel(t)}</b></div><div class="detail"><small>مدة الدراسة</small><b>${trainingLabel(t)} في Drama Road</b></div><div class="detail"><small>المهارات</small><b>${t.skills?.length?esc(t.skills.join('، ')):'—'}</b></div><div class="detail"><small>ملاحظات</small><b>${esc(t.notes||'—')}</b></div></div><a class="whatsapp-talent-btn" href="${whatsappTalentUrl(t)}" target="_blank" rel="noopener"><span>واتساب</span><div><b>طلب هذه الموهبة</b><small>إرسال الاسم والصورة إلى معهد Drama Road</small></div></a><div class="profile-projects" id="profileProjectsSection"><h4>المشاريع والأعمال</h4>${tp.length?tp.map(p=>`<div class="mini-project"><b>${esc(p.title)}</b><div>${esc(p.type||'')} ${p.year?'• '+p.year:''}${p.role?' • '+esc(p.role):''}</div>${p.url?`<a class="project-link-btn" href="${esc(p.url)}" target="_blank" rel="noopener">▶ مشاهدة المشروع</a>`:''}</div>`).join(''):'<p style="color:#888">لا توجد مشاريع مضافة بعد.</p>'}</div></div></div>`;$('#profileModal').classList.add('show');$('#profileModal').setAttribute('aria-hidden','false');history.replaceState(null,'',talentProfileUrl(t))}
...
function renderProjects(){if(!projects.length){$('#projectsGrid').innerHTML='<div class="empty">لم تتم إضافة مشاريع بعد. يمكنك إضافة أول مشروع من قسم الإدارة.</div>';return}$('#projectsGrid').innerHTML=projects.map(p=>{const t=talents.find(x=>x.id===p.talentId);return `<article class="project-card"><span class="eyebrow">${esc(p.type||'PROJECT')}</span><h3>${esc(p.title)}</h3><p><b>${esc(t?.name||'موهبة')}</b>${p.role?' • '+esc(p.role):''}${p.year?' • '+p.year:''}</p>${p.url?`<a class="project-link-btn" href="${esc(p.url)}" target="_blank" rel="noopener">▶ مشاهدة المشروع</a>`:''}</article>`}).join('')}
