...
function talentCard(t){const skills=t.skills?.length?t.skills.map(s=>`<span class="chip">${esc(s)}</span>`).join(''):'';return `<article class="talent-card" data-id="${esc(t.id)}"><div class="talent-photo"><img src="${esc(t.image||'logo.webp')}" alt="${esc(t.name)}" onerror="this.src='logo.webp'"></div><div class="talent-body"><h3>${esc(t.name)}</h3><div class="meta"><span class="chip red">${ageLabel(t)}</span>${t.heightCm?`<span class="chip">${t.heightCm} سم</span>`:''}${skills}</div><p class="talent-note">${esc(t.notes||'Drama Road Young Talent')}</p></div></article>`}
...
function renderProjects(){if(!projects.length){$('#projectsGrid').innerHTML='<div class="empty">لم تتم إضافة مشاريع بعد. يمكنك إضافة أول مشروع من قسم الإدارة.</div>';return}$('#projectsGrid').innerHTML=projects.map(p=>{const t=talents.find(x=>x.id===p.talentId);return `<article class="project-card project-card-with-photo"><div class="project-student-row"><img class="project-student-photo" src="${esc(t?.image||'logo.webp')}" alt="${esc(t?.name||'موهبة')}" onerror="this.src='logo.webp'"><div class="project-student-meta"><span class="eyebrow">${esc(p.type||'PROJECT')}</span><b>${esc(t?.name||'موهبة')}</b></div></div><h3>${esc(p.title)}</h3><p>${p.role?esc(p.role):''}${p.year?(p.role?' • ':'')+p.year:''}</p>${p.url?`<a class="project-link-btn" href="${esc(p.url)}" target="_blank" rel="noopener">▶ مشاهدة المشروع</a>`:''}</article>`}).join('')}
